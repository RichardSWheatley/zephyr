...
static int pdm_clock_settings_derive(const struct device *dev, struct dmic_cfg *dev_config)
{
	...
	ARRAY_FOR_EACH(settings_table, idx0) {
		if (pcm_rate != settings_table[idx0].pcm_rate) {
			continue;
		}

		osr = settings_table[idx0].osr;
		io_freq = pcm_rate * osr;
		if (IN_RANGE(io_freq, min_clk_freq, max_clk_freq) == false) {
			continue;
		}
		...
		ARRAY_FOR_EACH(freq_table, idx1) {
			...
			ret = am_hal_clkmgr_clock_config(AM_HAL_CLKMGR_CLK_ID_PLLPOSTDIV,
							 pdm_op_freq, NULL);
			if (ret != AM_HAL_STATUS_SUCCESS) {
				continue;
			}
