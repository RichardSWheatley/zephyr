...
NET_BUF_POOL_FIXED_DEFINE(ipc_tx_pool, IPC_TX_POOL_BUF_NUM,
                          IPC_TX_POOL_BUF_SIZE, 0, NULL);

static const struct mbox_dt_spec vnd_mbox_tx =
    MBOX_DT_SPEC_GET(DT_PATH(mbox_consumer), tx);
static const struct mbox_dt_spec vnd_mbox_rx =
...
int am_rss_mgr_ipc_shm_config(void)
{
    ...
    uint32_t config[] =
    {
        ...
        DT_REG_ADDR(DT_NODELABEL(sram_ipc)), DT_REG_SIZE(DT_NODELABEL(sram_ipc))
    };
